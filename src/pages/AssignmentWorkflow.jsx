        ...
        <div className="space-y-8 animate-fade-in pb-20">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/assignments')} className="p-2 hover:bg-light-surface-hover rounded-lg transition-colors">
                        <ArrowLeft className="w-6 h-6 text-light-text" />
                    </button>
                    <div>
                        <h1 className="text-3xl font-bold text-light-text">Workflow Test Console</h1>
                        <p className="text-light-text-secondary">Testing Steps for Assignment #{id.slice(-6).toUpperCase()}</p>
                    </div>
                </div>
            ...
            {/* INFO CARD */}
            <div className="bg-white border border-light-border rounded-2xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <h3 className="text-xs font-semibold text-light-text-tertiary mb-2 uppercase">Customer</h3>
                    <p className="text-light-text font-medium flex items-center gap-2"><User className="w-4 h-4" /> {customer.fullName || 'N/A'}</p>
                    <p className="text-light-text-secondary text-sm">{customer.mobileNumber}</p>
                    <p className="text-light-text-secondary text-sm flex items-start gap-1 mt-1"><MapPin className="w-3 h-3 mt-1" /> {customer.address}</p>
                </div>
                <div>
                    <h3 className="text-xs font-semibold text-light-text-tertiary mb-2 uppercase">Technician</h3>
                    <p className="text-light-text font-medium flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-400" /> {technician.fullName || 'Unassigned'}</p>
                    <p className="text-light-text-secondary text-sm">{technician.email}</p>
                </div>
                <div>
                    <h3 className="text-xs font-semibold text-light-text-tertiary mb-2 uppercase">Service / Booking</h3>
                    <p className="text-light-text font-medium">{booking.serviceId?.title || 'General Service'}</p>
                    <p className="text-light-text-secondary text-sm">Status: <span className="uppercase text-primary-400">{assignment.status}</span></p>
                </div>
            </div>

            {/* STEP 1: PREPARATION */}
            <div className="bg-white border border-light-border rounded-2xl overflow-hidden">
                <div className="bg-light-bg p-4 border-b border-light-border flex items-center gap-2">
                    <div className="w-8 h-8 rounded bg-blue-500/10 flex items-center justify-center text-blue-400 font-bold">1</div>
                    <h2 className="text-lg font-bold text-light-text">Treatment Preparation</h2>
                </div>
                <div className="p-6">
                    {/* List */}
                    <div className="space-y-3 mb-6">
                        {assignment.treatmentPreparation?.length === 0 && <p className="text-light-text-secondary italic">No preparation items added.</p>}
                        {assignment.treatmentPreparation?.map((item) => (
                            <div key={item._id} className="flex items-center justify-between p-3 bg-light-bg rounded-lg border border-light-border">
                                <div>
                                    <p className="text-light-text font-medium">{item.chemicals} <span className="text-light-text-tertiary">({item.quantity})</span></p>
                                </div>
                    ...
                    {/* Form */}
                    <form onSubmit={handleAddPrep} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-light-bg/50 p-4 rounded-xl">
                        <input
                            placeholder="Chemical Name"
                            className="bg-light-bg border border-light-border rounded p-2 text-light-text"
            ...
            <div className="bg-white border border-light-border rounded-2xl overflow-hidden">
                <div className="bg-light-bg p-4 border-b border-light-border flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold text-light-text">Site Pictures (Cloudinary)</h2>
                </div>
                <div className="p-6">
                    {/* Gallery */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        {assignment.applyTreatment?.sitePictures?.length === 0 && <p className="col-span-4 text-light-text-secondary italic">No pictures uploaded.</p>}
                        {assignment.applyTreatment?.sitePictures?.map((pic) => (
                            <div key={pic._id} className="relative group aspect-square bg-light-bg rounded-lg overflow-hidden border border-light-border">
                    ...
                    {/* Upload */}
                    <form onSubmit={handleUploadPicture} className="flex gap-4 bg-light-bg/50 p-4 rounded-xl items-center">
                        <input
                            id="file-upload"
                            type="file"
                            accept="image/*"
                            onChange={e => setPictureFile(e.target.files[0])}
                            className="text-light-text file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-500/10 file:text-purple-400 hover:file:bg-purple-500/20"
                        />
            ...
            <div className="bg-white border border-light-border rounded-2xl overflow-hidden">
                <div className="bg-light-bg p-4 border-b border-light-border flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold text-light-text">Service Execution</h2>
                </div>
                <div className="p-6">
                    <div className="space-y-3">
                        {booking.subServiceIds?.length === 0 && <p className="text-light-text-secondary italic">No services listed.</p>}
                        {booking.subServiceIds?.map((subItem, idx) => {
                            const service = subItem.serviceId || {};
                            const isCompleted = subItem.status === 'completed';
                            return (
                                <div key={idx} className={`flex items-center justify-between p-4 rounded-lg border transition-all ${isCompleted ? 'bg-green-500/5 border-green-500/20' : 'bg-light-bg border-light-border'}`}>
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-lg bg-white border border-light-border overflow-hidden">
                                            {service.metaImage || service.image ? (
                                                ...
                                                <div className="w-full h-full flex items-center justify-center"><Beaker className="w-5 h-5 text-light-text-tertiary" /></div>
                                            )}
                                        </div>
                                        <div>
                                            <h3 className="font-semibold text-light-text">{service.title || 'Unknown Service'}</h3>
                                            <p className="text-sm text-light-text-secondary">₹{service.startingPrice}</p>
                                        </div>
                                    </div>
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all ${isCompleted
                                            ...
                                            : 'bg-white border border-light-border text-light-text-secondary hover:text-light-text hover:border-primary-500'
                                            }`}
                                    >
            ...
            <div className="bg-white border border-light-border rounded-2xl overflow-hidden">
                <div className="bg-light-bg p-4 border-b border-light-border flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-bold text-light-text">Payment Collection</h2>
                </div>
                <div className="p-6">
                    {/* List */}
                    <div className="space-y-3 mb-6">
                        {(!assignment.paymentCollection || !assignment.paymentCollection.amount) && <p className="text-light-text-secondary italic">No payments recorded.</p>}

                        {assignment.paymentCollection && assignment.paymentCollection.amount > 0 && (
                            <div className="flex items-center justify-between p-3 bg-light-bg rounded-lg border border-light-border">
                                <div>
                                    <p className="text-light-text font-medium flex items-center gap-2">
                                        <span className="text-green-400">₹{assignment.paymentCollection.amount}</span>
                                        <span className="text-xs bg-white px-2 py-0.5 rounded capitalize border border-light-border">{assignment.paymentCollection.paymentMethod}</span>
                                    ...
                                    <p className="text-xs text-light-text-secondary">{new Date(assignment.paymentCollection.paymentDate || Date.now()).toLocaleDateString()}</p>
                                </div>
                    ...
                    {/* Form */}
                    <form onSubmit={handleAddPayment} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-light-bg/50 p-4 rounded-xl">
                        <input
                            type="number"
                            placeholder="Amount"
                            className="bg-light-bg border border-light-border rounded p-2 text-light-text"
