...
const Header = ({ toggleSidebar }) => {
    ...
    return (
        <>
            <header className={`fixed top-0 left-0 md:left-72 right-0 h-[70px] border-b flex items-center justify-between px-4 sm:px-6 z-30 transition-all duration-300 bg-white border-light-border`}>
                {/* Left Section */}
                <div className="flex items-center gap-2 sm:gap-4 flex-1">
                    {/* Mobile Menu Button */}
                    <button
                        onClick={toggleSidebar}
                        className={`md:hidden flex items-center justify-center w-10 h-10 rounded-lg transition-colors duration-200 hover:bg-light-surface-hover text-light-text`}
                    >
                ...
                <div className="flex items-center gap-3">
                    ...
                    {/* Divider */}
                    <div className={`w-px h-8 hidden sm:block bg-light-border`} />

                    {/* User Profile */}
                    <div className={`hidden sm:flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors duration-200 hover:bg-light-surface-hover`}>
                    ...
                    <button
                        ...
                        className={`flex items-center justify-center w-10 h-10 rounded-lg transition-all duration-200 hover:bg-red-500/10 text-light-text hover:text-red-600`}
                        title="Sign Out"
                    >
            ...
            {showLogoutConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
                    <div className={`w-full max-w-sm rounded-2xl shadow-2xl p-6 transform transition-all scale-100 bg-white`}>
                        <div className="flex flex-col items-center text-center gap-4">
                            <div className="w-12 h-12 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-600 dark:text-red-400">
                                <LogOut className="w-6 h-6" />
                            </div>
                            <div>
                                <h3 className={`text-lg font-bold mb-1 text-light-text`}>Confirm Logout</h3>
                            ...
                            <div className="flex items-center gap-3 w-full mt-2">
                                <button
                                    ...
                                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-colors bg-gray-100 text-gray-700 hover:bg-gray-200`}
                                >
                                    Cancel
                                </button>
                                <button
